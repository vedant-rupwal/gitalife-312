...
import { Plus, Trash2, Loader2, Upload, ExternalLink, Pencil, X } from "lucide-react";
    ...
    <div className="rounded-2xl bg-white border border-navy/8 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { resetForm(); setShowForm(!showForm); }} className="flex items-center gap-1 rounded-lg bg-saffron px-3 py-2 font-heading text-xs font-semibold text-white"><Plus className="h-3.5 w-3.5" />New</button>
        </div>
      </div>
      {showForm && (
        <form onSubmit={save} className="space-y-3 mb-4 rounded-xl bg-cream p-4">
          <div className="flex items-center justify-between gap-3">
            <h4 className="font-heading text-sm font-bold text-navy">{editingEventId ? "Edit Event" : "Create Event"}</h4>
            {editingEventId && (
              <button type="button" onClick={() => { resetForm(); setShowForm(false); }} className="flex items-center gap-1 rounded-lg bg-navy/5 px-3 py-2 font-heading text-xs font-semibold text-navy hover:bg-navy/10"><X className="h-3.5 w-3.5" />Cancel</button>
            )}
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            {!editingEventId && form.recurrence_frequency !== "none" && (
          ...
          <button type="submit" disabled={saving} className="flex items-center gap-2 rounded-xl bg-navy px-5 py-3 font-heading text-sm font-semibold text-white disabled:opacity-60">
